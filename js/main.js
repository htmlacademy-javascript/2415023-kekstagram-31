...
const usedId = [];
const createPost = () => {
  const randomId = getRandomInteger(PHOTO_ID.min, PHOTO_ID.max);
  const randomLikesNumber = getRandomInteger(LIKES_NUMBER.min, LIKES_NUMBER.max - 1);
  ...
  if (!usedId.includes(randomId)) {
    ...
    return {
      ...
      description: DESCRIPTIONS[randomId - 1],
...
while (usedId.length < POSTS_NUMBER) {
